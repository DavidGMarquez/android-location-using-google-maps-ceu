...
public class TestAvisos extends TestCase {
	public void testCrearAvisos() {
		...
		try {
			aviso = new Aviso(nombreAviso, descripcionAviso, tipoAviso,
					usuario, puntoMapa);
		} catch (Exception e) {
			fail("Esto no es normal!!");
		}
		try {
			aviso = new Aviso(nombreAviso, tipoAviso, descripcionAviso,
					usuario, fechacreacion, puntoMapa, calificacion, votaciones);
		} catch (Exception e) {
			fail("Esto no es normal!!");
		}
	...
	public void testAvisoSinNombre() throws NombreAvisoException {
		...
		PuntoMapa pm = null;
		try {
			Aviso aviso = new Aviso("", "Aviso sin nombre", u, pm);
		...
		try {
			Aviso aviso = new Aviso("", "Aviso sin nombre", "", u,
					fechacreacion, pm, 0, 0);
			fail("Deberia haber fallado");
		} catch (Exception e) {
			// Se ha lanzado la excepción por intentar meter un aviso sin nombre
			e.printStackTrace();
	...
	public void testFormatoCoordenadas() throws FormatoCoordenadasException {
		...
		try {
			PuntoMapa pm = new PuntoMapa("hola", "0");
			;
			fail("Deberia haber fallado");
		} catch (FormatoCoordenadasException e) {
			// Se ha lanzado la excepción por intentar meter unas coordenadas
			// con la latitud incorrecta
			e.printStackTrace();
		}

		try {
			PuntoMapa pm = new PuntoMapa("0", "bye");
			;
			fail("Deberia haber fallado");
		} catch (FormatoCoordenadasException e) {
			// Se ha lanzado la excepción por intentar meter unas coordenadas
			// con la longitud incorrecta
			e.printStackTrace();
		}

		try {
			PuntoMapa pm = new PuntoMapa("91", "0");
			;
			fail("Deberia haber fallado");
		} catch (FormatoCoordenadasException e) {
			// Se ha lanzado la excepción por intentar meter unas coordenadas
			// con la latitud inexistente
			e.printStackTrace();
		}

		try {
			PuntoMapa pm = new PuntoMapa("0", "181");
			;
			fail("Deberia haber fallado");
		} catch (FormatoCoordenadasException e) {
			// Se ha lanzado la excepción por intentar meter unas coordenadas
			// con la longitud inexistente
			e.printStackTrace();
		}
	}

	public void testPuntoMapaAGeoPoint() {
		try {
			PuntoMapa pm = new PuntoMapa("40.83", "-3.17");
			GeoPoint gp = new GeoPoint(pm.latitudE6(), pm.longitudE6());
			PuntoMapa pm2 = new PuntoMapa(new Double((double) gp
					.getLatitudeE6() / 1000000).toString(), new Double(
					(double) gp.getLongitudeE6() / 1000000).toString());

			// Comprobar que al pasar del PuntoMapa al GeoPoint y de vuelta al
			// PuntoMapa, no han cambiado las coordenadas
			assertEquals(pm.getLatitud(), pm2.getLatitud());
			assertEquals(pm.getLongitud(), pm2.getLongitud());
